...
//		bcbrt.setIcon(new ImageIcon("Webp.net-resizeimage (39).png"));
		...
		bpersantage.setBounds(473, 239, 79, 60);
		...
		equation.setBounds(28, 45, 521, 45);
	...
	private void fontchange()
	{
		...
		if(l1.getText().length()>20&&l1.getText().length()<26)
		{
			font=30;
		}
		if(l1.getText().length()<=30 &&l1.getText().length()>=26)
		{
			font=25;
		}
		...
		else if(source==badd)
		{
			...
			else if(str.contains("^"))
			{
				try
							...
							double sum=Math.pow(Double.parseDouble(str.substring(0,n)),Double.parseDouble(str.substring(n+1,str.length())));
							number2=sum;
